 ...
 **/
export class Search {
	public $options: Partial<SearchOptions> = {};
	 ...
	 * @chainable
	**/
	set(options: Partial<SearchOptions>) {
	...
	**/
	...
	find(session: EditSession) {
		...
		return firstRange as Range | null;
	}
	...
	**/
	...
	parseReplaceString(replaceString: string) {
		var CharCode = {
			DollarSign: 36,
			Ampersand: 38,
			Digit0: 48,
	 ...
	 * @returns {String}
	**/
	replace(input: string, replacement: string = '') {
		var options = this.$options;

		var re = this.$assembleRegExp(options) as RegExp | false;
		if (options.$isMultiLine)
			return replacement;

		util.assert(!Array.isArray(re), "$assembleRegExp returned array for non-multiline search");
		...
		if (options.preserveCase) {
			...
			for (var i = Math.min(input.length, input.length); i--; ) {
				...
				if (ch && ch.toLowerCase() != ch)
					replacementArr[i] = replacementArr[i].toUpperCase();
				else
					replacementArr[i] = replacementArr[i].toLowerCase();
			}
	 ...
	 */
	$assembleRegExp(options: Partial<SearchOptions>, $disableFakeMultiline?: boolean) {
	...
	$isMultilineSearch(options: Partial<SearchOptions>): boolean {
		return options.re && /\\r\\n|\\r|\\n/.test(options.re.source) && options.regExp && !options.$isMultiLine;
	}

	$multiLineForward(session: EditSession, re: RegExp, start: number, last: number) {
	...
	$multiLineBackward(session: EditSession, re: RegExp, endIndex: number, start: number, first: number) {
		var line: string | undefined,
			chunk = chunkEnd(session, start),
	 ...
	 */
	$matchIterator(session: EditSession, options: Partial<SearchOptions>) {
		var re_ = this.$assembleRegExp(options);
		if (!re_)
			return false;
		...
		var supportsUnicodeFlag = (re_ as RegExp).unicode;

		var range = options.range;
		var start_ = options.start as Range | Point;
		if (!start_)
			start_ = range ? range[backwards ? "end" : "start"] : session.selection.getRange();

		if ((start_ as Range).start)
			start_ = (start_ as Range)[skipCurrent != backwards ? "end" : "start"];
		
		const start = start_ as Point;

		var firstRow = range ? range.start.row : 0;
		var lastRow = range ? range.end.row : session.getLength() - 1;
		var forEachInLine: (row: number, index: number,
			callback: (sr: number, sc: number, er: number, ec: number) => boolean) => boolean | void;

		if (backwards) {
			var forEach = function(callback: (sr: number, sc: number, er: number, ec: number) => boolean) {
				var row = start.row;
		...
		const re = re_ as RegExp;

		if (options.$isMultiLine) {
			const re = re_ as RegExp[];
			var len = re.length;
			forEachInLine = function(row, offset, callback) {
			...
			forEachInLine = function(row, endIndex, callback) {
				if (mtSearch) {
			...
			forEachInLine = function(row, startIndex, callback) {
 ...
 * @return {string}
 */
function addWordBoundary(needle: string, options: Partial<SearchOptions>) {
...
function multiLineBackwardMatch(line: string, re: RegExp, endMargin: number) {
	var match = null;
	var from = 0;
...
function chunkEnd(session: any, start: number) {
	var base = 5000,
