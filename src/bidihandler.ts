...
export class BidiHandler {
	...
	/* current screen row */
	currentRow: number = -1;
	bidiUtil: typeof bidiUtil = bidiUtil;
	...
	**/
	isBidiRow(screenRow: number, docRow?: number, splitIndex?: number) {
		if (!this.seenBidi)
			return false;
	...
	updateRowLine(docRow?: number, splitIndex?: number) {
		if (docRow === undefined)
	...
	**/
	getSelections(startCol: number, endCol: number) {
