...
import {Tokenizer as BaseTokenizer} from "./tokenizer";
...
export class Tokenizer extends BaseTokenizer {
	 ...
	 * @returns {Object}
	 **/
	getLineTokens(line: string, startState: string | any[]) {
		...
		var token: {type: string | null, value: string, state?: string} = {
			type: null,
			value: "",
			state: currentState,
		};
		...
		while (match = re.exec(line)) {
			...
			for (var i = 0; i < match.length-2; i++) {
				...
				if (rule.next) {
					...
					if (!state) {
						globalThis.console && console.error && console.error(currentState, "doesn't exist");
						currentState = "start";
						state = this.states[currentState];
					}
