...
import {Delta,IRange,Point,Range} from "./range"
import type {Selection} from "./selection"

export interface UndoManager {
	hasUndo: typeof UndoManager.prototype.canUndo;
	hasRedo: typeof UndoManager.prototype.canRedo;
	isClean: typeof UndoManager.prototype.isAtBookmark;
	markClean: typeof UndoManager.prototype.bookmark;
}
...
export class UndoManager {
	 ...
	 * @param {number} [rev]
	 */
	addSelection(selection: Range | Range[], rev?: number) {
	 ...
	 * @param {object} json 
	 */
	fromJSON(json: { $redoStack: Delta[][]; $undoStack: Delta[][] }) {
		this.reset();
 ...
 * @param {number} pos
 */
function rearrangeUndoStack(stack: Delta[][], pos: number) {
	for (var i = pos; i--; ) {
 ...
 * @param {Delta} delta
 */
function $updateMarkers(this: any, delta: Delta) {
 ...
 * @return {string}
 */
function stringifyRange(r: IRange) {
...
function swapGroups(ds1: Delta[], ds2: Delta[]) {
	...
	// @ts-ignore
	ds1.selectionAfter = ds2.selectionAfter = void 0;
	return [ds2, ds1];
}
 ...
 * @param {Delta} c1
 */
function xform(d1: Delta, c1: Delta) {
	var i1 = d1.action == "insert";
 ...
 * @param {number} dir
 */
function shift(d1: IRange, d2: IRange, dir: number) {
 ...
 * @param {number} dir
 */
function shiftPos(pos: Point, start: Point, end: Point, dir: number) {
 ...
 * @return {Delta}
 */
function splitDelta(c: Delta, pos: Point) {
	var lines = c.lines;
	var end = c.end;
	c.end = clonePos(pos);    
 ...
 * @param {Delta} d
 */
function moveDeltasByOne(redoStack: Delta[][], d: Delta) {
	d = cloneDelta(d);
...
function rebaseRedoStack(redoStack: Delta[][], deltaSets: Delta[][]) {
