...
function is(token: any, type: string): boolean {
 ...
 *
 * @param {EditSession} session - The editor session containing the code to format
 */

export function beautify(session: EditSession) {
	var iterator = new TokenIterator(session, 0, 0);
	var token = iterator.getCurrentToken();
	var tabString = session.getTabString();
	var singletonTags_ = singletonTags;
	var blockTags_ = blockTags;
	var formatOptions_ = formatOptions || {};
	...
	while (token !== null) {
		...
		if (typeof token !== "undefined") {
			...
			if (is(token, "tag-open")) {
				...
				if (nextToken)
					inBlock = (blockTags_.indexOf(nextToken.value) !== -1);
			...
			if (value) {
				...
				// trim value after opening paren
					...
					if (value.substr(0, 1) === "{") {
						...
						if (code.substr(-1) !== '[' && code.trimEnd().substr(-1) === '[') {
							trimCode();
							spaceBefore = false;
						} else if (code.trimEnd().substr(-1) === ')') {
							trimCode();
						} else {
							trimLine();
						}
				...
				// space after colon or comma
					...
					if (value.match(/^(,)$/) && curlyDepth>0 && roundDepth===0 && formatOptions_.lineBreaksAfterCommasInCurlyBlock) {
						rowsToAdd++;
					} else {
						spaceAfter = true;
						breakBefore = false;
				...
				if ((is(token, "tag-close") && (inBlock || blockTags_.indexOf(tagName) !== -1)) || (is(token, "doctype") && value === ">")) {
				...
				// html indentation
				if (nextToken && singletonTags_.indexOf(nextToken.value) === -1) {
