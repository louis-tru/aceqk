 ...
 * @param {Editor} editor
 */
function onChangeSelection(e: any, editor: Editor) {
	var lead = editor.getSelection().lead as RemoveReadonly<Anchor>;
...
function onCommandEmitted(commadEvent: {command: {name: string}, editor: Editor}) {
 ...
 * @param {Editor} editor
 */
function onChange(delta: Delta, editor: Editor) {
	var session = editor.session;
	// session.$bidiHandler.currentRow = null;
	session.$bidiHandler.currentRow = -1;
