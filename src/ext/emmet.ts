...
export class AceEmmetEditor {
	 ...
	 * @param {Editor} editor
	 */
	setupContext(editor: Editor) {
		this.ace = editor;
		this.indentation = editor.session.getTabString();
		if (!emmet)
			// @ts-ignore
			emmet = globalThis["emmet"];
	 ...
	 */
	createSelection(start: number, end?: number) {
		var doc = this.ace.session.doc;
		this.ace.selection.setRange({
			start: doc.indexToPosition(start),
			end: doc.indexToPosition(end|| start)
		});
	}
	 ...
	 */
	replaceContent(value: string, start?: number, end?: number, noIndent?: boolean) {
		if (end == null)
	...
	$updateTabstops(value: string) {
		...
		var tabstopOptions = {
			tabstop: function(data: {group: string, placeholder: string, start: number}) {
 ...
 */
export function runEmmetCommand(this: any, editor: Editor): TimeoutResult | boolean {
	...
	try {
		...
		editor._signal("changeStatus", typeof e == "string" ? e : e.message || '', editor);
		config.warn(e);
		result = false;
	}
	return result;
 ...
 */
export function updateCommands(editor: Editor, enabled?: boolean) {
	if (enabled) {
...
var onChangeMode = function(e: { enableEmmet?: boolean }, target: Editor) {
	var editor = target;
	if (!editor)
		return;
 ...
 */
export function load(cb?: () => void): boolean {
 ...
 */
export function setCore(e: string | object) {
	if (typeof e == "string")
		emmetPath = e;
	else
		emmet = e;
