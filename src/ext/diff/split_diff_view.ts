...
import {BaseDiffView} from "./base_diff_view";
...
import type {DiffHighlight} from "./base_diff_view";
...
export class SplitDiffView extends BaseDiffView {
	 ...
	 */
	constructor(diffModel?: DiffModel) {
		diffModel = diffModel || {};
		super();
		this.init(diffModel);
	}
	...
	onScroll(e: number, session: EditSession) {
		this.syncScroll(this.sessionA === session ? this.editorA.renderer : this.editorB.renderer);
	}

	private $syncScroll?: boolean;
	private scrollSetBy?: VirtualRenderer;
	private scrollSetAt: number = Infinity;
	private scrollA?: number;
	private scrollB?: number;

	/**
	 * @param {import("../../virtual_renderer").VirtualRenderer} renderer
	 */
	syncScroll(renderer: VirtualRenderer) {
	...
	$detachEditorEventHandlers(editor: Editor) {
