...
import {EditSession, EditSessionEvents} from "../../edit_session";
...
export interface BaseDiffView extends OptionsProvider<DiffViewOptions> {}

export class BaseDiffView {
	diffSession: {sessionA: EditSession, sessionB: EditSession, chunks: DiffChunk[]};
	editorA: EditorExt;
	editorB: EditorExt;
	activeEditor: EditorExt;
	otherEditor: EditorExt;
	 ...
	 */
	constructor(inlineDiffEditor?: boolean, container?: Text) {
	...
	/**
	 * @param {DiffModel} [diffModel] - The model for the diff view.
	 */
	$setupModels(diffModel: DiffModel) {
		...
		var diffEditorOptions: Partial<EditorOptions> = ({
			scrollPastEnd: 0.5,
	 ...
	 * @param {string} [value]
	 */
	$setupModel(session?: EditSession, value?: string) {
	 ...
	 */
	setDiffSession(session: { sessionA: any; sessionB: EditSession; chunks: DiffChunk[] }) {
		if (this.diffSession) {
			this.$detachSessionsEventHandlers();
			this.clearSelectionMarkers();
		}
		this.diffSession = session;
		(this as any).sessionA = (this as any).sessionB = null;
		if (this.diffSession) {
	...
	selectionRangeA: Range | null = null;
	selectionRangeB: Range | null = null;
	...
	setupScrollbars() {
		 ...
		 */
		const setupScrollBar = (renderer: Renderer & {$scrollDecorator?: ScrollDiffDecorator}) => {
			setTimeout(() => {
	...
	$setScrollBarDecorators(renderer: Renderer & {$scrollDecorator?: ScrollDiffDecorator}) {
		...
		renderer.scrollBarV.element.style.marginBottom = renderer.scrollBarH.getHeight();
		renderer.scrollBarV.element.style.align = "bottom";
	}

	$resetDecorators(renderer: Renderer & {$scrollDecorator?: Decorator}) {
		if (renderer.$scrollDecorator) {
			renderer.$scrollDecorator.destroy();
		}
	...
	updateScrollBarDecorators() {
		 ...
		 */
		const updateDecorators = (editor: EditorExt, change: DiffChunk) => {
			if (!editor) {
				return;
			}
	 ...
	 * @return {DiffChunk[]}
	 */
	$diffLines(val1: string[], val2: string[]) {
	 ...
	 */
	setProvider(provider: IDiffProvider) {
	 ...
	 */
	$addWidget(session: EditSession, w: {rowCount: number; rowsAbove: number; row: number;}) {
	...
	onSelect(e: any, selection: Selection) {
	...
	syncSelect(selection: Selection) {
	...
	updateSelectionMarker(marker: SyncSelectionMarker, session: EditSession, range: Range) {
		marker.setRange(range);
		session._signal("changeFrontMarker", void 0, session);
	}

	$syncingFold: boolean = false;

	/**
	 * @param ev
	 * @param {EditSession} session
	 */
	onChangeFold(ev: Parameters<EditSessionEvents["changeFold"]>[0], session: EditSession) {
		var fold = ev.data as Fold;
	...
	destroy() {
		...
		self.editorA = self.editorB = null;
	}
	 ...
	 */
	transformRange(range: IRange, isOriginal: boolean) {
	 ...
	 */
	transformPosition(pos: Point, isOriginal: boolean) {
		...
		var [from, to] = isOriginal ? ["old", "new"] as FromTo[] : ["new", "old"] as FromTo[];
		var deltaChar = 0;
	 ...
	 * @param {number} toLine
	 */
	$getDeltaIndent(fromEditSession: EditSession, toEditSession: EditSession, fromLine: number, toLine: number) {
	 ...
	 * @param {number} line
	 */
	$getIndent(editSession: EditSession, line: number) {
	 ...
	 * @return {number}
	 */
	findChunkIndex(chunks: DiffChunk[], row: number, isOriginal: boolean) {
	...
	searchHighlight(selection: Selection) {
		...
		otherSession._signal("changeBackMarker", void 0, otherSession);
	}
...
config.defineOptions(BaseDiffView.prototype, "DiffView", {
	...
	folding: {
		set: function(this: BaseDiffView, value: boolean) {
			...
			if (!value) {
				...
				if (this.chunks) {
					this.chunks.forEach(x=>{
						posA.push({start: x.old.start, end: x.old.end});
						posB.push({start: x.new.start, end: x.new.end});
					});
				}
...
var emptyGutterRenderer =  {
	getText: function name(params: any) {
		return "";
	},
	getWidth() {
		return 0;
...
export class DiffChunk {
	...
	constructor(originalRange: Range, modifiedRange: Range, charChanges?: {
		originalStartLineNumber: number, originalStartColumn: number,
		originalEndLineNumber: number, originalEndColumn: number, modifiedStartLineNumber: number,
		modifiedStartColumn: number, modifiedEndLineNumber: number, modifiedEndColumn: number
	}[]) {
...
export class DiffHighlight implements MarkerLike {
	 ...
	 * @param type
	 */
	constructor(diffView: BaseDiffView, type: number) {
		/**@type{number}*/this.id;
		this.diffView = diffView;
		this.type = type;
	}

	update(html: string[], markerLayer: Marker, session: EditSession, config: LayerConfig) {
		let dir: 'new' | 'old', operation, opOperation;
