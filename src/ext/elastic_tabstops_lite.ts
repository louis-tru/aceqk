...
export class ElasticTabstopsLite {
	 ...
	 * @returns {number[]}
	 */
	$selectionColumnsForRow(row: number) {
	 ...
	 */
	$setBlockCellWidthsToMax(cellWidths: number[][]) {
	 ...
	 * @returns {number}
	 */
	$rightmostSelectionInCell(selectionColumns: number[], cellRightEdge: number) {
		var rightmost = 0;
	 ...
	 * @param {number[]} widths
	 */
	$adjustRow(row: number, widths: number[]) {
		...
		for (var i = 0, l = expandedSet.length; i < l; i++) {
			...
			var partialLine = this.$editor.session.getLine(row).substring(0, it );
