...
export class FoldLine {
	 ...
	 */
	walk(callback: (placeholder: string | null, row: number, column: number, lastEnd: number, isNewRow: boolean) => boolean | void,
			endRow: number, endColumn: number) 
	{
		var lastEnd = 0,
			folds = this.folds,
			fold,
			cmp, stop: any, isNewRow = true;
		...
		for (var i = 0; i < folds.length; i++) {
			...
			stop = !stop && callback(fold.placeholder, fold.start.row, fold.start.column, lastEnd, false);
	 ...
	 */
	getNextFoldTo(row: number, column: number): { fold: Fold, kind: string } | null {
		var fold, cmp;
